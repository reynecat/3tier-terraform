#!/bin/bash
# aws/scripts/backup-instance-init-planb.sh
# Plan B (Pilot Light): RDS → Azure Blob Storage 직접 백업
# S3 미사용 (AWS 리전 마비 시 S3도 접근 불가)
...
# 5. Azure 오래된 백업 정리 (30일 이상 - Lifecycle Policy로 자동 관리)
# Lifecycle Policy로 자동 삭제되므로 수동 정리 불필요
...
echo ""
echo "수동 백업 실행:"
echo "  /usr/local/bin/mysql-backup-to-azure.sh"
echo ""
